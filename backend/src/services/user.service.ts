...
export async function buildUsersExportWorkbook(search?: string, role?: string, status?: string): Promise<Buffer> {
  const statusBool: boolean | undefined = status === "active" ? true : status === "inactive" ? false : undefined;
  ...
  return buildUsersWorkbook(users);
}
