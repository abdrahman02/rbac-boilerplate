    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        Object.entries(grouped).map(([resource, perms]) => (
          <div key={resource} className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
              ...
              <tbody className="divide-y divide-gray-200">
                {perms.map((perm) => (
                  <tr key={perm.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{new Date(perm.createdAt).toLocaleDateString()}</td>
      ...
      <PermissionModal isOpen={modalOpen} onClose={() => setModalOpen(false)} permission={selectedPermission} />
    </div>
  );
}
